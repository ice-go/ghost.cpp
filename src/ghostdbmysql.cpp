...
CCallableGameAdd *CGHostDBMySQL :: ThreadedGameAdd( string server, string map, string gamename, string ownername, uint32_t duration, uint32_t gamestate, string creatorname, string creatorserver, vector<string> chatlobbylog, vector<string> chatgamelog )
{
	void *Connection = GetIdleConnection( );
	...
	CCallableGameAdd *Callable = new CMySQLCallableGameAdd( server, map, gamename, ownername, duration, gamestate, creatorname, creatorserver, chatlobbylog, chatgamelog, Connection, m_BotID, m_Server, m_Database, m_User, m_Password, m_Port );
	CreateThread( Callable );
	m_OutstandingCallables++;
	return Callable;
}
...
uint32_t MySQLGameAdd( void *conn, string *error, uint32_t botid, string server, string map, string gamename, string ownername, uint32_t duration, uint32_t gamestate, string creatorname, string creatorserver, vector<string> chatlobbylog, vector<string> chatgamelog )
{
	uint32_t RowID = 0;
	string EscServer = MySQLEscapeString( conn, server );
	...
	string Query = "INSERT INTO games ( botid, server, map, date, gamename, ownername, duration, gamestate, creatorname, creatorserver ) VALUES ( " + UTIL_ToString( botid ) + ", '" + EscServer + "', '" + EscMap + "', UNIX_TIMESTAMP(), '" + EscGameName + "', '" + EscOwnerName + "', " + UTIL_ToString( duration ) + ", " + UTIL_ToString( gamestate ) + ", '" + EscCreatorName + "', '" + EscCreatorServer + "' )";
	...
	if (RowID != 0)
	{
		string EscChatLobbyLog;
		
		for( vector<string> :: iterator i = chatlobbylog.begin( ); i != chatlobbylog.end( ); i++ )
		EscChatLobbyLog.append( (*i) + "\r\n" );
		
		EscChatLobbyLog = MySQLEscapeString( conn, EscChatLobbyLog );
		string Query2 = "INSERT INTO chatlobbylogs ( gameid, chatlog ) VALUES ( " + UTIL_ToString(RowID) + ", '" + EscChatLobbyLog + "')";
		
		mysql_real_query( (MYSQL *)conn, Query2.c_str( ), Query2.size( ) );
		
		string EscChatGameLog;
		
		for( vector<string> :: iterator i = chatgamelog.begin( ); i != chatgamelog.end( ); i++ )
		EscChatGameLog.append( (*i) + "\r\n" );
		
		EscChatGameLog = MySQLEscapeString( conn, EscChatGameLog );
		string Query3 = "INSERT INTO chatgamelogs ( gameid, chatlog ) VALUES ( " + UTIL_ToString(RowID) + ", '" + EscChatGameLog + "')";
		
		mysql_real_query( (MYSQL *)conn, Query3.c_str( ), Query3.size( ) );
	}
